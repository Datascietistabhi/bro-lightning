...
2.  `bind-addr` informs Core Lightning to bind itself to port 9833.
    This is needed for the subsequent `statictor` to work.
    9833 is the normal Lightning Network port, so this setting may already be present.
...
HiddenServicePort 1234 127.0.0.1:9833
```
...
1. A local address which lightningd can tell Tor to connect to when
   connections come in, e.g. `bind-addr=127.0.0.1:9833`.
