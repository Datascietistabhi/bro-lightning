 ...
 **network**=*NETWORK*
Select the network parameters (*brocoin*, *testnet*, *signet*, or *regtest*).
This is not valid within the per-network configuration file.

 **mainnet**
Alias for *network=brocoin*.
...
the discovered IP with TCP port 9833 as announced address. If unset and you
open TCP port 9833 on your router towords your node, your node will remain
...
port 9833 on IPv4 and IPv6, and will announce it to peers if it seems
...
For this to work you need to forward the TCP port 9833 to your node.
...
If 'PORT' is not specified, the default port 9833 is used for mainnet
...
and this will be used to configure a Tor hidden service for port 9833
...
is distinguished by ending in *.onion*. *PORT* defaults to 9833.
