...
to `/usr/local/etc/brocoin.conf`, maybe also `testnet=1`.

Configure lightningd: copy `/usr/local/etc/lightningd-brocoin.conf.sample` to
`/usr/local/etc/lightningd-brocoin.conf` and edit according to your needs.
...
autogen automake libsodium 'python3.withPackages (p: [p.brocoinlib])' \
valgrind --run make
```
...
**Note**: Edit your `~/Library/Application\ Support/Bitcoin/brocoin.conf`
