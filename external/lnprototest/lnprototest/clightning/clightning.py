...
class Runner(lnprototest.Runner):
    ...
    def start(self) -> None:
        ...
        self.brocoind = Bitcoind(self.directory)
        try:
            self.brocoind.start()
        ...
        self.proc = subprocess.Popen(
            [
                ...
                "--bitcoin-rpcport={}".format(self.brocoind.port),
                "--log-level=debug",
                "--log-file=log",
            ]
            + self.startup_flags
    ...
    def getblockheight(self) -> int:
        return self.brocoind.rpc.getblockcount()

    def trim_blocks(self, newheight: int) -> None:
        h = self.brocoind.rpc.getblockhash(newheight + 1)
        self.brocoind.rpc.invalidateblock(h)

    def add_blocks(self, event: Event, txs: List[str], n: int) -> None:
        for tx in txs:
            self.brocoind.rpc.sendrawtransaction(tx)
        self.brocoind.rpc.generatetoaddress(n, self.brocoind.rpc.getnewaddress())
    ...
    def expect_tx(self, event: Event, txid: str) -> None:
        ...
        try:
            wait_for(lambda: revtxid in self.brocoind.rpc.getrawmempool())
        except ValueError:
            raise EventError(
                event,
                "Did not broadcast the txid {}, just {}".format(
                    revtxid,
                    [
                        (txid, self.brocoind.rpc.getrawtransaction(txid))
                        for txid in self.brocoind.rpc.getrawmempool()
                    ],
                ),
            )
