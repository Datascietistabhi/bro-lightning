...
def test_lightningd_still_loading(node_factory, bitcoind, executor):
    ...
    def mock_getblock(r):
        conf_file = os.path.join(bitcoind.brocoin_dir, 'brocoin.conf')
...
def test_bech32_funding(node_factory, chainparams):
    ...
    wallettx = l1.brocoin.rpc.getrawtransaction(wallettxid, True)
    fundingtx = l1.brocoin.rpc.decoderawtransaction(res['fundingtx'])
...
def test_withdraw_misc(node_factory, bitcoind, chainparams):
    ...
    for i in range(10):
        l1.brocoin.rpc.sendtoaddress(addr, amount / 10**8)
    ...
    waddr = l1.brocoin.getnewaddress()
    ...
    unspent = l1.brocoin.rpc.listunspent(0)
