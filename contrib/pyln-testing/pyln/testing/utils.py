...
class BitcoinD(TailableProc):
    ...
    def __init__(self, bitcoin_dir="/tmp/bitcoind-test", rpcport=None):
        ...
        self.conf_file = os.path.join(bitcoin_dir, 'brocoin.conf')
...
class LightningD(TailableProc):
    def __init__(self, lightning_dir, bitcoindproxy, port=9833, random_hsm=False, node_id=0):
...
class LightningNode(object):
    ...
    def openchannel(self, remote_node, capacity=FUNDAMOUNT, addrtype="p2sh-segwit", confirm=True, wait_for_announce=True, connect=True):
        ...
        if confirm or wait_for_announce:
            self.brocoin.generate_block(1, wait_for_mempool=res['txid'])

        if wait_for_announce:
            self.brocoin.generate_block(5)
    ...
    def fundwallet(self, sats, addrtype="p2sh-segwit", mine_block=True):
        ...
        txid = self.brocoin.rpc.sendtoaddress(addr, sats / 10**8)
        if mine_block:
            self.brocoin.generate_block(1)
    ...
    def fundbalancedchannel(self, remote_node, total_capacity, announce=True):
        ...
        blockid = self.brocoin.generate_block(1, wait_for_mempool=res['txid'])[0]

        # Generate the scid.
        for i, txid in enumerate(self.brocoin.rpc.getblock(blockid)['tx']):
            if txid == res['txid']:
                txnum = i

        return '{}x{}x{}'.format(self.brocoin.rpc.getblockcount(), txnum, res['outnum'])
    ...
    def fundchannel(self, l2, amount=FUNDAMOUNT, wait_for_active=True,
        ...
        self.brocoin.rpc.sendtoaddress(addr, (amount + 1000000) / 10**8)
        self.brocoin.generate_block(1)
        ...
        scid = "{}x{}x{}".format(self.brocoin.rpc.getblockcount(),
                                 txnum, res['outnum'])
    ...
    def wait_for_channel_onchain(self, peerid):
        ...
        wait_for(lambda: txid in self.brocoin.rpc.getrawmempool())
    ...
    def wait_for_onchaind_broadcast(self, name, resolve=None):
        ...
        txid = self.brocoin.rpc.decoderawtransaction(rawtx, True)['txid']
...
class NodeFactory(object):
    ...
    def get_node(self, node_id=None, options=None, dbfile=None,
        ...
        node = self.node_cls(
            node_id, lightning_dir, self.brocoind, self.executor, self.valgrind, db=db,
